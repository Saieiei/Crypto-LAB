...
def main():
    ...
    print(f"\nReceived Encrypted Message: {binascii.hexlify(ciphertext).decode('utf-8')}")

    key = b'sixteenB'
    cipher = DES.new(key, DES.MODE_ECB)

    # Display each round of decryption
    for round_num in range(16, 0, -1):
        # Decrypt the message for this round
        partial_decrypted_text = unpad(cipher.decrypt(ciphertext), DES.block_size)
        print(f"Round {round_num} Decrypted Message: {partial_decrypted_text.decode('utf-8')}")
